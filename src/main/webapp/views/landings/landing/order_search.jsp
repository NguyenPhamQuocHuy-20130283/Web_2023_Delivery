<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FedEx</title>
    <link rel="stylesheet" href="../css/order_search.css">
</head>
<body>
    <div class="navbar">
        <div class="logo">
<<<<<<<< Updated upstream:src/main/webapp/views/web/landings/landing/order_search.html
<<<<<<< Updated upstream:src/main/webapp/views/web/landings/landing/order_search.jsp
=======
<<<<<<<< Updated upstream:src/main/webapp/views/web/landings/landing/order_search.html
>>>>>>> Stashed changes:src/main/webapp/views/landings/landing/order_search.jsp
          <a href="index.html"><img src="../assets/asset 0.png" alt=""></a>
========
            <a href="index.jsp"><img src="../assets/asset 0.png" alt=""></a>
>>>>>>>> Stashed changes:src/main/webapp/views/web/landings/landing/order_search.jsp
<<<<<<< Updated upstream:src/main/webapp/views/web/landings/landing/order_search.jsp
=======
========
            <a href="index.jsp"><img src="../assets/asset 0.png" alt=""></a>
>>>>>>>> Stashed changes:src/main/webapp/views/web/landings/landing/order_search.jsp
>>>>>>> Stashed changes:src/main/webapp/views/landings/landing/order_search.jsp
        </div>
        <div class="dropdown">
          <button class="dropbtn">Dịch vụ </button>
          <div class="dropdown-content">
            <a href="#">Giao hàng nhanh </a>
            <a href="#">Kho bãi và xử lí hàng hoá</a>
    
          </div>
        </div>
        <div class="dropdown">
<<<<<<<< Updated upstream:src/main/webapp/views/web/landings/landing/order_search.html
          <button class="dropbtn">Giới thiệu </button>
          <div class="dropdown-content">
            <a href="#">Về FedEx</a>
            <a href="#">Tin tức</a>
            <a href="#">Quan hệ với nhà đầu tư</a>
            <a href="#">Liên hệ</a>
          </div>
        </div>
        <div class="dropdown">
          <button class="dropbtn">Hỗ trợ </button>
          <div class="dropdown-content">
            <a href="customer_support.html">Câu hỏi thường gặp</a>
            <a href="customer_sp_detail.html">Hướng dẫn sử dụng</a>
            <a href="term-of-use.html">Điều khoản sử dụng</a>
            <a href="privacy-page.html">Chính sách bảo mật</a>
            <a href="">Chính sách vận chuyển</a>
          </div>
========
            <button class="dropbtn">Giới thiệu </button>
            <div class="dropdown-content">
                <a href="about.jsp">Về FedEx</a>
                <a href="#">Quan hệ với nhà đầu tư</a>
                <a href="#">Liên hệ</a>
            </div>
        </div>
        <div class="dropdown">
            <button class="dropbtn">Hỗ trợ </button>
            <div class="dropdown-content">
                <a href="customer_support.jsp">Câu hỏi thường gặp</a>
                <a href="customer_sp_detail.jsp">Hướng dẫn sử dụng</a>
                <a href="term-of-use.html">Điều khoản sử dụng</a>
                <a href="privacy-page.html">Chính sách bảo mật</a>
                <a href="not_allowed_packet.jsp">Hàng hoá không nhận vận chuyển</a>
            </div>
>>>>>>>> Stashed changes:src/main/webapp/views/web/landings/landing/order_search.jsp
        </div>
        <div class="dropdown">
          <button class="dropbtn">Thông tin</button>
          <div class="dropdown-content">
            <a href="">Tin tức</a>
            <a href="">Bảng giá</a>
            <a href="">Khuyến mãi</a>
          </div>
        </div>
    
        <div class="dropdown">
<<<<<<<< Updated upstream:src/main/webapp/views/web/landings/landing/order_search.html
          <button class="dropbtn">Đăng nhập/Đăng kí <i class="bi bi-person-circle"></i></button>
          <div class="dropdown-content">
            <a href="login.html">Đăng nhập</a>
            <a href="signup.html">Đăng kí</a>
          </div>
========
            <button class="dropbtn">Đăng nhập/Đăng kí <i class="bi bi-person-circle"></i></button>
            <div class="dropdown-content">
                <a href="login.jsp">Đăng nhập</a>
                <a href="signup.html">Đăng kí</a>
            </div>
>>>>>>>> Stashed changes:src/main/webapp/views/web/landings/landing/order_search.jsp
        </div>
        <div class="search-container">
          <input type="search" placeholder="Mã vận chuyển" id="search-input">
          <a href="order_search.html" id="search-icon">
            <i class="bi bi-search"></i>
          </a>
        </div>
      </div>
      <!-- end of header -->
<div class="main">
    <div class="order-info">
        <div class="header-order">
            <div class="title-first">
                Thông tin đơn hàng
            </div>
            <div class="table-row">
                <div class="title">
                    Mã đơn hàng:
                </div>
                <div class="value">
                    G8GLU3G8
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Ngày lấy dự kiến:
                </div>
                <div class="value">
                    12/12/2020
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Ngày giao dự kiến:
                </div>
                <div class="value">
                    12/12/2020
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Trạng thái:
                </div>
                <div class="value status">
                 Hoàn thành
                </div>
            </div>

        </div>
        <div class="header-order">
            <div class="title-first">
                Người gửi
            </div>
            <div class="table-row">
                <div class="title">
                    Họ và tên:
                </div>
                <div class="value">
                    xxx 7544
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Số điện thoại:
                </div>
                <div class="value">
                    xxxx 7544
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Địa chỉ:
                </div>
                <div class="value">
                    xxxx Quận 5 Hồ Chí Minh
                </div>
            </div>
            
        </div>
        <div class="header-order">
            <div class="title-first">
                Người gửi
            </div>
            <div class="table-row">
                <div class="title">
                    Họ và tên:
                </div>
                <div class="value">
                    xxx 7544
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Số điện thoại:
                </div>
                <div class="value">
                    xxxx 7544
                </div>
            </div>
            <div class="table-row">
                <div class="title">
                    Địa chỉ:
                </div>
                <div class="value">
                    xxxx Quận 5 Hồ Chí Minh
                </div>
            </div>
            
        </div>
    </div>
    <div class="order-history">
        <div class="card-order-container">
            <div class="card-order-header"><span>Lịch sử đơn hàng</span></div>
            <div class="card-order-content">
                <table>
                    <tr>
                        <th class="first">Thứ 7, 14/10/2023</th>
                        <th class="middle">Chi tiết</th>
                        <th class="last">Thời gian</th>
                    </tr>
                    <tr>
                        <td>Giao hàng thành công </td>
                        <td>
                            Đơn hàng được giao thành công tại xxxx Thành phố Dĩ An Bình Dương</td>
                        <td>11:31</td>
                    </tr>
                    <tr>
                        <td>Cuộc gọi</td>
                        <td>Nguyễn Văn Hoàng Hải gọi điện cho xxxx manh - xxxx 7291
                            Nghe máy 5s</td>
                        <td>11:29</td>
                    </tr>
                    <tr>
                        <td>Đang giao hàng lần 2</td>
                        <td>Đơn hàng đang giao đến xxxx Thành phố Dĩ An Bình Dương</td>
                        <td>
                            08:19</td>
                    </tr>
                    <tr>
                        <td>Sẵn sàng giao hàng</td>
                        <td>Đơn hàng sẵn sàng được giao tại Bưu Cục 34 Nguyễn Thị Khắp-Dĩ An-Bình Dương </td>
                        <td>08:19</td>
                    </tr>

                </table>
            </div>
        </div>
    </div>
</div>
        <!-- end of order search -->

</body>
</html>